...
public class JedisShiroCache<K, V> implements Cache<K, V> {
    ...
    public V get(K key) throws CacheException {
        ...
        try {
            byteValue = redisUtil.getValueByKey(DB_INDEX, byteKey);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public V put(K key, V value) throws CacheException {
        V previos = get(key);
        try {
            redisUtil.saveValueByKey(DB_INDEX, SerializeUtil.serialize(buildCacheKey(key)),
    ...
    public V remove(K key) throws CacheException {
        V previos = get(key);
        try {
            redisUtil.deleteByKey(DB_INDEX, SerializeUtil.serialize(buildCacheKey(key)));
        } catch (Exception e) {
            e.printStackTrace();
